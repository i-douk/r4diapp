...
const getSubscriptions = async () => {
  ...
  const podcasterIds = [
    ...new Set(subscriptionsData?.map(sub => sub.podcaster_id)),
  ]
  ...
  if (usersError)
    useErrorStore().setError({ error: usersError, customCode: status })
  users.value = usersData

  // Fetch podcasters
  const {
    data: podcastersData,
    error: podcastersError,
    status: podStatus,
  } = await supabase.from('podcasters').select().in('id', podcasterIds)
  if (podcastersError)
    useErrorStore().setError({ error: podcastersError, customCode: podStatus })
  podcasters.value = podcastersData

  // Populate subscriptinsDataToshow with user and podcaster names, stipend, and paid status
  subscriptinsDataToshow.value =
