...
podcastsRouter.post('/:username', tokenExtractor, async ( req : Request, res : Response) => {
    ...
    const podcaster = await models.Podcaster.findOne({ where : { username : req.params.username}});
    const activePodcaster = await models.ActivePodcasterSession.findByPk(podcaster?.id)
...
podcastsRouter.patch('/:id', tokenExtractor, async( req : Request, res: Response) => {
...
// edit podcast
podcastsRouter.put('/:id', tokenExtractor, async( req : Request, res: Response) => {
  const podcastId = req.params.id;
  const {name , description , transcribed , urls } = req.body;
  let podcastToEdit = await models.Podcast.findByPk(podcastId);
  if(podcastToEdit){
